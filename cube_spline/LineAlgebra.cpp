	...
	for (size_t i = 0; i < inRow; i++) {
		...
		for (size_t j = 0; j < inColumn; j++) {
			...
			if (inPtrMatr[i][j] != 0) {
				if (indicator_first) {
					if (inRow - j == 0) {
						std::cout << "= " << inPtrRight[i] << std::endl;
					} else {
						if (inPtrMatr[i][j] == 1) {
							std::cout << "x" << j + 1 << " "; 
						} else {
...
void StaggeredMatrix(type** ptrMatr, type* ptrRight, int N) {
	...
	bool indicator_denerate;

	type *x = nullptr;  
	type *HelpVector = nullptr; 
	...
	for (size_t k = 0; k < N; k++) {
		...
		for (size_t m = k + 1; m < N; m++) {
			if (fabs(ptrMatr[m][k]) > fabs(ptrMatr[i_max][k])) {      // ��������� ��������� � ������� i, ��� ������ �������������
				i_max = m;
			}
		}

		if (fabs(ptrMatr[i_max][k]) < ZeroEps) {       // ���� ������������ ������� ����? ��� - �����.
			...
			indicator_denerate = true;    // ������� �� ���������
			if (i_max != k) {
				HelpVector = ptrMatr[i_max];       // ������ ����� ���������� � ������� ����������
				ptrMatr[i_max] = ptrMatr[k];       //<=>
				ptrMatr[k] = HelpVector;           //����� ������� ������

				HelpVar = ptrRight[i_max];       // ������ ������� ������ �����
		...
		if (indicator_denerate = true) {
			for (size_t i = k + 1; i < N; i++) {
				double coef = ptrMatr[i][k] / ptrMatr[k][k];     // ����� ������������ ������ ������
...
void VectorNevyazki(type** Matrix, type* vector, type* x, int N) {
	...
	std::cout << std::endl;
	std::cout << "����� ������� ������� ||b1 - b||_2 = " << norm << std::endl;
	std::cout << "����� ������� ������� ||b1 - b||_1 = " << norm_1 << std::endl;
	std::cout << "����� ������� ������� ||b1 - b||_inf = " << norm_infinity << std::endl;
